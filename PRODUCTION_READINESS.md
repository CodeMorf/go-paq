...
|---|---|---|
| Schema Drizzle | Verificado | `drizzle/schema.ts`, journal y SQL están versionados; `drizzle/schema.ts`, journal y SQL declaran 39 tablas y 25 migraciones; el verificador clean-schema se ejecutó pero no pudo completar el cleanup por `ER_DBACCESS_DENIED_ERROR`, por lo que no se afirma una instalación limpia completa; se retiró el SQL 0000 duplicado no referenciado. |
...
| Documentación API | Corregida | `/docs-api` refleja los endpoints REST verificados: `POST /api/v1/quotes`, `POST /api/v1/shipments`, `POST /api/v1/pickups`, `GET /api/v1/tracking/:trackingCode` y `GET /api/v1/webhook-deliveries`, todos con API key, versionado, rate limit y scopes. |
