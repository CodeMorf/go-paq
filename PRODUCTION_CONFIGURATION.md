...
La instalación limpia se intentó aplicando la secuencia completa del journal en una base temporal: el usuario pudo crear el esquema, pero el cleanup falló con `ER_DBACCESS_DENIED_ERROR` al eliminar la base. Por tanto, **39 tablas** y **25 migraciones** están verificadas por schema/journal/tests, pero la corrida clean-schema no se declara completada hasta disponer de privilegios bootstrap adecuados. El esquema mantiene `DOP` como moneda por defecto y `America/Santo_Domingo` como zona horaria.
