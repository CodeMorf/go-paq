...
function destinationForRole(role?: string) {
  ...
  if (normalized === 'CLIENT' || normalized === 'CUSTOMER') return '/portal/dashboard';
  if (normalized === 'DRIVER') return '/driver';
  if (['COUNTER', 'DISPATCHER', 'WAREHOUSE', 'BRANCH_MANAGER', 'MANAGER', 'SUPERVISOR'].includes(normalized)) return '/sucursal/dashboard';
...
export const LoginPage: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      go(destinationForRole(result.user?.role));
    } catch (err: any) {
  ...
  return <div className={shell}>
    <form onSubmit={submit} className={card}>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-400">Accede a tu portal logístico.</p>
      </div>
      ...
      <button type="button" className="mt-5 w-full text-sm text-indigo-300 hover:text-indigo-200" onClick={() => go('/register')}>Crear cuenta de cliente</button>
    </form>
  </div>;
};
...
export const RegisterPage: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.token) localStorage.setItem('gopaq_token', result.token);
      go('/portal/dashboard');
    } catch (err: any) {
  ...
  return <div className={shell}>
    <form onSubmit={submit} className={card}>
      ...
      <button type="button" className="mt-5 w-full text-sm text-indigo-300 hover:text-indigo-200" onClick={() => go('/login')}>Ya tengo una cuenta</button>
    </form>
  </div>;
};
