...
export const ProductionShipmentsManager: React.FC = () => {
  const { setIsNewShipmentModalOpen, setSelectedTracking, setCurrentSection, setActiveSubView } = useApp();
  ...
  const openTracking = (shipment: any) => { setSelectedTracking(shipment.tracking_number || shipment.trackingNumber); setCurrentSection('portal'); setActiveSubView('rastreo'); };
...
export const ProductionClientRegistration: React.FC = () => {
  ...
  return <div className="space-y-6"><PanelHeader icon={<UserRound className="h-6 w-6 text-indigo-600" />} title="Registro de clientes" description="Alta real de clientes en la organización actual. No se generan lockers, API keys ni coordenadas ficticias." /><ErrorBox message={error} /><Card><form onSubmit={submit} className="grid gap-4 md:grid-cols-2"><label className="text-xs font-bold">Nombre<input required minLength={2} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm" /></label><label className="text-xs font-bold">Empresa opcional<input value={form.companyName} onChange={(event) => setForm({ ...form, companyName: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm" /></label><label className="text-xs font-bold">Correo<input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm" /></label><label className="text-xs font-bold">Teléfono<input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm" /></label><label className="text-xs font-bold">RNC / identificación<input value={form.rncTaxId} onChange={(event) => setForm({ ...form, rncTaxId: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm" /></label><label className="text-xs font-bold">Sucursal<select value={form.branchId} onChange={(event) => setForm({ ...form, branchId: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm"><option value="">Sin sucursal asignada</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><div className="md:col-span-2"><Button type="submit" variant="primary" disabled={busy}>{busy ? 'Guardando…' : 'Registrar cliente'}</Button></div></form></Card></div>;
};
