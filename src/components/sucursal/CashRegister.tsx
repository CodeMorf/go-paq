...
export const CashRegister: React.FC = () => {
  ...
  const close = async (event: React.FormEvent) => { event.preventDefault(); if (!branch) return; setSubmitting(true); const key = idempotencyKey.current || crypto.randomUUID(); idempotencyKey.current = key; const result = await ApiClient.closeBranchCash(branch.id, { totalCash, totalPos, totalTransfers, notes }, key); setSubmitting(false); if (!result.success) { setError(result.error); return; } idempotencyKey.current = null; addToast('success', 'Cierre guardado', `Cierre ${result.summary.id} confirmado por GoPaq.`); setError(''); };
