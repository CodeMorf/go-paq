  ...
  return (
    <Card className="border-0 bg-white shadow-sm">
      <CardHeader className="flex-row items-start justify-between gap-4">
        <div><CardTitle className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-orange-500" /> Manifest operativi</CardTitle><p className="mt-1 text-sm text-slate-500">Apri, sigilla e riconcilia i trasferimenti di filiale.</p></div>
        <div className="flex items-center gap-2"><select aria-label="Direzione manifest" value={direction} onChange={(event) => setDirection(event.target.value as typeof direction)} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs text-slate-700"><option value="outbound">Partenza</option><option value="inbound">Arrivo</option><option value="transfer">Trasferimento</option></select><Button size="sm" className="bg-orange-500 text-white hover:bg-orange-400" disabled={create.isPending} onClick={() => create.mutate({ direction })}><Plus className="mr-1 h-4 w-4" />Apri</Button></div>
      </CardHeader>
      <CardContent className="space-y-3">
        {manifests.isLoading && <p className="text-sm text-slate-500">Caricamento manifest…</p>}
