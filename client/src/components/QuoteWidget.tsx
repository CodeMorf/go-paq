  ...
  return (
    <Card className="border-border bg-card text-card-foreground shadow-xl">
      ...
      <CardContent>
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4 rounded-2xl bg-muted p-4"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Total estimado</p><p className="mt-1 text-3xl font-black text-primary">{quote.data ? formatMoney(quote.data.total, "DOP") : "—"}</p></div><Button className="bg-primary text-primary-foreground hover:opacity-90" onClick={() => setLocation("/cliente")}>Solicitar envío</Button></div>
      </CardContent>
    </Card>
  );
}
