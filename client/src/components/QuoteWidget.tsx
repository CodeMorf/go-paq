...
export default function QuoteWidget() {
  ...
  return (
    <Card className="border-border bg-card text-card-foreground shadow-xl">
      <CardHeader><p className="text-xs font-bold uppercase tracking-[.22em] text-primary">Cotización en vivo</p><CardTitle className="mt-2 text-2xl">Calcula tu envío</CardTitle><p className="text-sm text-muted-foreground">Estimación transparente en DOP (RD$), basada en peso, volumen, distancia y recargo de combustible.</p></CardHeader>
      <CardContent>
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4 rounded-2xl bg-muted p-4"><div><p className="text-xs uppercase tracking-[.18em] text-muted-foreground">Total estimado</p><p className="mt-1 text-3xl font-black text-primary">{quote.data ? `RD$ ${quote.data.total.toFixed(2)}` : "—"}</p></div><Button className="bg-primary text-primary-foreground hover:opacity-90">Solicitar envío</Button></div>
      </CardContent>
    </Card>
  );
}
