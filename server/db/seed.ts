...
export async function runDemoSeeds() {
  ...
  await transactionAsync(async (tx) => {
    ...
    await tx.execute(`INSERT INTO drivers (id, organization_id, branch_id, user_id, name, email, phone, license_number, vehicle_type, vehicle_plate, status, current_lat, current_lng, rating, active, created_at, updated_at) VALUES ('drv-demo', 'org-demo', 'br-demo-central', 'usr-demo-driver', 'Demo Driver', 'demo.driver@gopaq.local', '', 'DEMO', 'Furgoneta Demo', 'DEMO-000', 'available', 18.4861, -69.9312, 5, 1, ?, ?) ON CONFLICT (id) DO NOTHING`, [now, now]);
      ...
      await tx.execute(`INSERT INTO rates_matrix (id, organization_id, rule_code, service_type, origin_zone, dest_zone, base_rate, per_kg_rate, per_vol_rate, min_charge, pricing_mode, weight_unit, included_weight, additional_weight_step, additional_weight_rate, included_distance_km, distance_rate, surcharges_json, currency, active, created_at, updated_at) VALUES ('rate-demo-moving', 'org-demo', 'DEMO-MUDANZA', 'mudanza', '*', '*', 4500, 0, 850, 4500, 'hybrid', 'kg', 1, 1, 0, 0, 85, '{"floor_no_elevator":{"type":"unit","value":600},"crew_member":{"type":"unit","value":1200}}', 'DOP', 1, ?, ?) ON CONFLICT (id) DO NOTHING`, [now, now]);
      await tx.execute(`INSERT INTO rates_matrix (id, organization_id, rule_code, service_type, origin_zone, dest_zone, base_rate, per_kg_rate, per_vol_rate, min_charge, pricing_mode, weight_unit, included_weight, additional_weight_step, additional_weight_rate, included_distance_km, distance_rate, surcharges_json, currency, active, created_at, updated_at) VALUES ('rate-demo-heavy', 'org-demo', 'DEMO-CARGA', 'carga_pesada', '*', '*', 8500, 18, 500, 8500, 'base_plus_weight', 'kg', 1, 1, 18, 0, 0, '{"pallet":{"type":"unit","value":750},"equipment":{"type":"fixed","value":2500}}', 'DOP', 1, ?, ?) ON CONFLICT (id) DO NOTHING`, [now, now]);
