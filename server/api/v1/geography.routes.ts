...
geographyRouter.get('/', asyncHandler(async (req, res) => {
  ...
  const country = await queryOneAsync<any>('SELECT id, iso2, iso3, name, official_name FROM countries WHERE iso2 = ? AND active = TRUE', [iso2]);
  if (!country) return res.status(404).json({ success: false, error: 'País no encontrado.' });

  const provinceRows = await queryAllAsync<any>('SELECT id, code, name, capital, active FROM provinces WHERE country_id = ? AND active = TRUE ORDER BY name ASC', [country.id]);
  const zoneRows = await queryAllAsync<any>(`SELECT id, province_id, code, name, zone_number, description FROM service_zones WHERE active = TRUE AND province_id IN (${provinceRows.length ? provinceRows.map(() => '?').join(',') : "''"}) ORDER BY zone_number ASC`, provinceRows.map((province) => province.id));
