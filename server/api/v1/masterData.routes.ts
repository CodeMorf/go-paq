...
const rateSchema = z.object({
  ...
  baseRate: z.coerce.number().min(0).max(100000000), perKgRate: z.coerce.number().min(0).max(100000000).default(0), perVolRate: z.coerce.number().min(0).max(100000000).default(0), minCharge: z.coerce.number().min(0).max(100000000), maxWeight: z.coerce.number().positive().max(100000).nullable().optional(),
...
ratesRouter.post('/', authenticate, requireRole(adminRoles), requireScope('rates:write'), asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  await transactionAsync(async (tx) => {
    await tx.execute(`INSERT INTO rates_matrix (id, organization_id, rule_code, service_type, service_variant, origin_zone, dest_zone, base_rate, per_kg_rate, per_vol_rate, min_charge, pricing_mode, weight_unit, included_weight, additional_weight_step, additional_weight_rate, included_distance_km, distance_rate, max_weight, currency, priority, client_id, branch_id, tiers_json, surcharges_json, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`, [id, req.organizationId, ruleCode, input.serviceType, input.serviceVariant || null, input.originZone, input.destZone, input.baseRate, input.perKgRate, input.perVolRate, input.minCharge, input.pricingMode, input.weightUnit, input.includedWeight, input.additionalWeightStep, input.additionalWeightRate ?? null, input.includedDistanceKm, input.distanceRate, input.maxWeight ?? null, input.currency, input.priority, input.clientId || null, input.branchId || null, JSON.stringify(input.tiers), JSON.stringify(input.surcharges), now, now]);
...
ratesRouter.patch('/:id', authenticate, requireRole(adminRoles), requireScope('rates:write'), asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  const now = new Date().toISOString();
  await transactionAsync(async (tx) => {
    await tx.execute(`UPDATE rates_matrix SET rule_code = ?, service_type = ?, service_variant = ?, origin_zone = ?, dest_zone = ?, base_rate = ?, per_kg_rate = ?, per_vol_rate = ?, min_charge = ?, pricing_mode = ?, weight_unit = ?, included_weight = ?, additional_weight_step = ?, additional_weight_rate = ?, included_distance_km = ?, distance_rate = ?, max_weight = ?, currency = ?, priority = ?, client_id = ?, branch_id = ?, tiers_json = ?, surcharges_json = ?, updated_at = ? WHERE id = ? AND organization_id = ? AND active = 1`, [input.ruleCode || null, input.serviceType, input.serviceVariant || null, input.originZone, input.destZone, input.baseRate, input.perKgRate, input.perVolRate, input.minCharge, input.pricingMode, input.weightUnit, input.includedWeight, input.additionalWeightStep, input.additionalWeightRate ?? null, input.includedDistanceKm, input.distanceRate, input.maxWeight ?? null, input.currency, input.priority, input.clientId || null, input.branchId || null, JSON.stringify(input.tiers), JSON.stringify(input.surcharges), now, req.params.id, req.organizationId]);
